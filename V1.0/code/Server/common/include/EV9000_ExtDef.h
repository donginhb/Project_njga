...
#define EV9000_LONG_LONG_STRING_LEN   256+4   //最长字符串长度
...
#define EV9000APP_VIDEOENCODE_TYPE_H265       0x40
 		
#define	EV9000APP_VIDEOENCODE_TYPE_HIK        0x500
#define	EV9000APP_VIDEOENCODE_TYPE_DAH        0x501
#define	EV9000APP_VIDEOENCODE_TYPE_WL         0x502 /* 东方网力解码库 */
#define	EV9000APP_VIDEOENCODE_TYPE_H3C        0x503 /* 东方网力解码库 */
...
//逻辑点位
typedef struct EV9000_GBLogicDeviceConfigEx
{
	unsigned int         nID;                                       //记录编号
...
}EV9000_GBLogicDeviceConfigEx, *LPEV9000_GBLogicDeviceConfigEx;
...
typedef struct EV9000_GBLogicDeviceConfig
{
	...
	unsigned int         nResved3;                                  //保留3
	char                 strResved4[EV9000_SHORT_STRING_LEN];       //保留4
	unsigned int         nGuard;                                    //设备端是否布防：1~布防；0~撤防。 遥信量有效，其他无效，默认0，整型
	char                 strValue[EV9000_LONG_LONG_STRING_LEN];     //遥信遥控：0~闭，1~开；遥测遥调：真实数据，浮点型；事件量命令量：字符串，默认0，字符串256
	unsigned int         nAlarmPriority;                            //0~无报警；1~3表示报警级别，默认0，整型
    char                 strUnit[EV9000_SHORT_STRING_LEN];          //遥测遥调时生效，默认空，字符串32
}EV9000_GBLogicDeviceConfig, *LPEV9000_GBLogicDeviceConfig;

//RCU状态信息
typedef struct EV9000_RCUDeviceStatus
{
    char                 strDeviceID[EV9000_IDCODE_LEN];                 //点位编号
	unsigned int         nStatus;                                        //设备状态
	unsigned int         nGuard;                                         //设备端是否布防：1~布防；0~撤防。 遥信量有效，其他无效，默认0，整型
    ...
    unsigned int         nAlarmPriority;                                 //0~无报警；1~3表示报警级别，默认0，整型
    char                 strUnit[EV9000_SHORT_STRING_LEN];               //遥测遥调时生效，默认空，字符串32
}EV9000_RCUDeviceStatus, *LPEV9000_RCUDeviceStatus;
...
typedef enum
{
	PLANACTION_PC = 0,        /* PC屏幕轮训 */
	PLANACTION_TVWALL,        /* 电视墙轮训 */
	PLANACTION_PRESET,        /* 预置位 */
	PLANACTION_ALARM_RECORD,  /* 报警录像 */
	PLANACTION_SEND_MAIL,     /* 发送邮件 */
	PLANACTION_CAPTURE_IMAGE, /* 抓图 */
	PLANACTION_EXECUTE_CRUISE,/* 执行巡航 */
	PLANACTION_CONTROL_RCU,   /* 执行RCU */
}PLANACTION_TYPE;

//RCU,strValue的取值
typedef enum
{
	EV9000_LOGICDEVICE_RCU_VALUE_CLOSE = 0,//远程，分
	EV9000_LOGICDEVICE_RCU_VALUE_OPEN,     //远程，合
}RCU_VALUE_TYPE;
...
typedef struct EV9000_LabelRecord
{
	unsigned int          nID;                                        //ID
	char                  strDeviceID[EV9000_IDCODE_LEN];
	unsigned int          nLabelTime;								 //标签时间
	unsigned int          nType;                                     //标签类型(保留)
	char                  strInfo[EV9000_LONG_STRING_LEN];           //标签内容
	unsigned int          nResved1;                                  //保留1
	char                  strResved2[EV9000_SHORT_STRING_LEN];       //保留2	
}EV9000_LabelRecord, *LPEV9000_LabelRecordConfig;

typedef struct
{
	short nX;                                                      //x坐标（CIF分辨率）
	short nY;                                                      //y坐标（CIF分辨率）
}EV9000_IVSBOX_POINT, *LPEV9000_IVSBOX_POINT;

//规则框
typedef struct  
{
	short nAreaType;                                               //2:无向线段;3:有向线段;1:区域
	short nPointCnt;                                               //定点数
	EV9000_IVSBOX_POINT Points[12];                                //定点位置
}EV9000_IVSBOX_AREABOX, *LPEV9000_IVSBOX_AREABOX;
...
typedef struct 
{
	...
	short nTop;                                                    //上边界
	short nDown;												   //下边界
}EV9000_IVSBOX_TARGETBOX, *LPEV9000_IVSBOX_TARGETBOX;

typedef struct
{
	unsigned char ucAvHead[8];                                     //标识符"WiscomV"
	unsigned int nLen;                                             //码流ID(结构体长度)
	unsigned int nFrameID;                                         //帧序号
	char         sChannelID[EV9000_IDCODE_LEN];                    //点位ID
	int          nAreaBoxCnt;                                      //规则框数量
	EV9000_IVSBOX_AREABOX  AreaBox[8];                             //规则框描述
	int          nTargatBoxCnt;                                    //运动框计数 <=50
	EV9000_IVSBOX_TARGETBOX lpTargetBox[50];                       //运动框描述 
}EV9000_IVSBOX_INFO, *LPEV9000_IVSBOX_INFO;
