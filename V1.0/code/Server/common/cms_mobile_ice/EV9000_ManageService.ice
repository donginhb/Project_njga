...
module EV9000MS
{
    ...
    interface EV9000MSI
    {
       ...
       ["ami","amd"] int  OMMPToCMSInfo(int nSendType, COMMNMSGHEAD tInCommMsg,BYTEBUFFER sInData,out COMMNMSGHEAD tOutCommMsg,out BYTEBUFFER sOutData);    
		...
		/*TSC通知mms任务结束*/
		int TscNotifyTaskEnd(string sTscTaskID);
		/* TSC注册，iRefresh=1表示刷新注册 */
		int TSCServerRegister(string sTscID, string sTscVideoIP, string sVideoIPEth, string sTscDeviceIP, string sDeviceIPEth, int iExpires, int iRefresh, int rtspserverport );
		/* RTSP注册，iRefresh=1表示刷新注册 */
		int RTSPServerRegister(string sTsuID, string sTsuVideoIP, string sVideoIPEth, string sTsuDeviceIP, string sDeviceIPEth, int iExpires, int iRefresh, int rtspserverport);
		 /* RTSP当前任务上报 */
      	int RTSPNotifyCurrentTask(RTSPTaskAttributeList tRTSPTaskAttribute);
		
        /* RTSP通知RTSP结束任务, 手机RTSP服务使用 */
      	int RTSPNotifyTaskEnd(string sTaskID);
