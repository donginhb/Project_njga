...
{ if (!soap->header)
    return SOAP_ERR;
  soap->header->SOAP_WSA(From) = (SOAP_WSA_SPACE(From)*)soap_malloc(soap, sizeof(SOAP_WSA_SPACE(From)));
...
{ if (!soap->header)
    return SOAP_ERR;
  if (replyTo)
  { soap->header->SOAP_WSA(ReplyTo) = (SOAP_WSA_SPACE(ReplyTo)*)soap_malloc(soap, sizeof(SOAP_WSA_SPACE(ReplyTo)));
...
{ if (!soap->header)
    return SOAP_ERR;
  if (faultTo)
  { soap->header->SOAP_WSA(FaultTo) = (SOAP_WSA_SPACE(FaultTo)*)soap_malloc(soap, sizeof(SOAP_WSA_SPACE(FaultTo)));
...
{ struct soap_wsa_data *data = (struct soap_wsa_data*)soap_lookup_plugin(soap, soap_wsa_id);
  ...
  { newheader->SOAP_WSA(RelatesTo) = (SOAP_WSA_SPACE(RelatesTo)*)soap_malloc(soap, sizeof(SOAP_WSA_SPACE(RelatesTo)));
      ...
      { struct SOAP_ENV__Detail *detail;
        *info = NULL;
        ...
        { switch (detail->__type)
	      ...
	      *info = ((SOAP_WSA_SPACE(ProblemAction)*)detail->fault)->Action;
	      break;
	    default:
	      break;
          }
  ...
  { case SOAP_WSA(InvalidAddressingHeader):
    ...
    case SOAP_WSA(ActionNotSupported):
      ...
      { soap->fault->detail->__type = SOAP_WSA_(SOAP_TYPE_,ProblemAction);
        soap->fault->detail->fault = (void*)soap_malloc(soap, sizeof(SOAP_WSA_SPACE(ProblemAction)));
        SOAP_WSA_(soap_default_,ProblemAction)(soap, (SOAP_WSA_SPACE(ProblemAction)*)soap->fault->detail->fault);
        ((SOAP_WSA_SPACE(ProblemAction)*)soap->fault->detail->fault)->Action = (char*)info;
      }
      else
      { soap->fault->SOAP_ENV__Detail->__type = SOAP_WSA_(SOAP_TYPE_,ProblemAction);
        soap->fault->SOAP_ENV__Detail->fault = (void*)soap_malloc(soap, sizeof(SOAP_WSA_SPACE(ProblemAction)));
        SOAP_WSA_(soap_default_,ProblemAction)(soap, (SOAP_WSA_SPACE(ProblemAction)*)soap->fault->SOAP_ENV__Detail->fault);
        ((SOAP_WSA_SPACE(ProblemAction)*)soap->fault->SOAP_ENV__Detail->fault)->Action = (char*)info;
      }
      break;
