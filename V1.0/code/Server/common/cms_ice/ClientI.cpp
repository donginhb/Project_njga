...
void AMI_ICE_HA_Client::ProgramRuning()
{
    if(m_our_status != 0)
    {
        ...
        strcpy(tmps8Ip, inet_ntoa(*(struct in_addr*)&g_BoardNetConfig.tCmsCommonVideoIP.tNetIP.dwIPAddr));
        printf("\n ping ip = %s \n", tmps8Ip);
        ...
        if (0 != tmpcPingClass.ping(5))
        {
           if (tmpcPingClass.GetTimeOut() <= 2)
           {
               ...
               printf("\r\n WQCLOG mast Network is ok \r\n");
                
           }
           else
           {
               printf("\r\n WQCLOG mast Network is down \r\n");
               S2MexchangeProc();
               setourstatus(0);
           }

           printf("\n cPingClass.GetTimeOut() = %d \n", tmpcPingClass.GetTimeOut());
        ...
        if(time(NULL) - m_HAtimer > 30 && m_our_status == 1)
        {
            ...
            printf("\n ping ip = %s \n", tmps8Ip);
            CPing  tmpcPingClass(tmps8Ip, 0);
            if (0 != tmpcPingClass.ping(3))
            {
                if (tmpcPingClass.GetTimeOut() <= 1)
                {
                    printf("\r\n WQCLOG salf Network down \r\n");
                    sleep(5);
                    return;
                ...
                {
                    printf("\r\n WQCLOG salf Network is ok but run S2MexchangeProc \r\n");
