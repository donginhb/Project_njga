...
int DBOperMysql::Connect(const char* szConPara, const char* dbPath, int iTimeOut)
{
    ...
    if(bRet)
    {
        ...
        #ifndef WIN32
        sqlset.clear();
        sqlset = "SET character_set_client='gb2312'";
        DBExecute(sqlset.c_str());
        sqlset.clear();
        sqlset = "SET character_set_connection='gb2312'";
        DBExecute(sqlset.c_str());
        sqlset.clear();
        sqlset = "SET character_set_results='gb2312'";
        DBExecute(sqlset.c_str());
        sqlset.clear();
...
bool DBOperMysql::ReConnect()
{
    if(m_pdb)
    {
        ...
        printf("\r\n EV9000DB_MOBILE m_strHost = %s,m_strUser = %s m_strPwd = %s m_strDBName = %s m_iPort = %d \r\n",m_strHost.c_str(),m_strUser.c_str(),m_strPwd.c_str(),m_strDBName.c_str(),m_iPort);
