...
__osip_nict_init(osip_nict_t** nict, osip_t* osip, osip_message_t* request)
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"allocating NICT context\n"));
...
__osip_nict_free(osip_nict_t* nict)
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"free nict ressource\n"));

    if (NULL != nict->destination)
    {
        osip_free(nict->destination);
        nict->destination = NULL;
    }
