...
nist_rcv_request(osip_transaction_t* nist, osip_event_t* evt)
{
    ...
    if (nist->state == NIST_PRE_TRYING)   /* announce new REQUEST */
    {
        ...
        {
            __osip_message_callback(OSIP_NIST_MESSAGE_RECEIVED, nist, nist->orig_request);
        }
        else if (MSG_IS_UPDATE(evt->sip))
        {
            __osip_message_callback(OSIP_NIST_UPDATE_RECEIVED, nist, nist->orig_request);
        }
        else if (MSG_IS_REFER(evt->sip))
        {
            __osip_message_callback(OSIP_NIST_REFER_RECEIVED, nist, nist->orig_request);
        }
        else
        {
        ...
        if (nist->last_response != NULL)    /* retransmit last response */
        {
            ...
            if (i != 0)
            {
                ...
                /*
                     ...
                     if (utc_time2 - utc_time1 > 0)
                     {
                         printf("\r\n nist_rcv_request time=%d \r\n", utc_time2 - utc_time1);
                     }
                     */
...
osip_nist_timeout_j_event(osip_transaction_t* nist, osip_event_t* evt)
{
    ...
    /*
     ...
     if (utc_time2 - utc_time1 > 0)
     {
         printf("\r\n osip_nist_timeout_j_event time=%d \r\n", utc_time2 - utc_time1);
     }
     */
