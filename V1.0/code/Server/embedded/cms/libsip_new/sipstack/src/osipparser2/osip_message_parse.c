...
__osip_message_startline_parsereq(osip_message_t* dest, const char* buf, const char** headers)
{
    ...
    *headers = buf;
    ...
    if (p2 - buf == 0)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"No space allowed here\n"));
        return OSIP_SYNTAXERROR;
    }
    ...
    if (p1 == NULL)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Uncompliant request-uri\n"));
    ...
    /* find the the version and the beginning of headers */
    {
        ...
        while ((*hp != '\r') && (*hp != '\n'))
        {
            if (*hp)
            {
                ...
                {
                    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"incorrect sip version string\n"));
                    osip_free(dest->sip_method);
                    dest->sip_method = NULL;
                    osip_uri_free(dest->req_uri);
                    dest->req_uri = NULL;
            ...
            {
                OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"No crlf found\n"));
                osip_free(dest->sip_method);
                dest->sip_method = NULL;
                osip_uri_free(dest->req_uri);
                dest->req_uri = NULL;
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Wrong version number\n"));
        }
...
__osip_find_next_occurence(const char* str, const char* buf, const char** index_of_str, const char* end_of_buf)
{
    ...
    *index_of_str = NULL;         /* AMD fix */
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_BUG, NULL, (char*)"This was probably an infinite loop?\n"));
    return OSIP_SYNTAXERROR;
}
...
__osip_find_next_crlf(const char* start_of_header, const char** end_of_header)
{
    ...
    *end_of_header = NULL;        /* AMD fix */
    ...
    while (('\r' != *soh) && ('\n' != *soh))
    {
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Final CRLF is missing\n"));
            return OSIP_SYNTAXERROR;
        }
    }
    ...
    if ((' ' == soh[1]) || ('\t' == soh[1]))
    {
        ...
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_BUG, NULL, (char*)"Message that contains LWS must be processed with osip_util_replace_all_lws(char *tmp) before being parsed.\n"));
        return -2;
    }
...
osip_message_set__header(osip_message_t* sip, const char* hname, const char* hvalue)
{
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_WARNING, NULL, (char*)"Could not set unknown header\n"));
        return OSIP_SUCCESS;
    }
...
msg_headers_parse(osip_message_t* sip, const char* start_of_header, const char** body)
{
    ...
    for (;;)
    {
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL, (char*)"SIP message does not end with CRLFCRLF\n"));
            return OSIP_SUCCESS;
        }
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"End of header Not found\n"));
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Malformed message\n"));
            return OSIP_SYNTAXERROR;
        }
...
msg_osip_body_parse(osip_message_t* sip, const char* start_of_buf, const char** next_body, size_t length)
{
    ...
    if (0 != osip_strcasecmp(sip->content_type->type, "multipart"))
    {
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Message was not receieved enterely. length=%i osip_body_len=%i\n", (int) length, (int) osip_body_len));
            return OSIP_SYNTAXERROR;
        }

        end_of_body = start_of_body + osip_body_len;
        tmp = (char*)osip_malloc(end_of_body - start_of_body + 2);
    ...
    *next_body = NULL;
    ...
    for (;;)
    {
        ...
        tmp = (char*)osip_malloc(body_len + 2);
...
_osip_message_parse(osip_message_t* sip, const char* buf, size_t length, int sipfrag)
{
    ...
    tmp = (char*)osip_malloc(length + 2);

    if (tmp == NULL)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Could not allocate memory.\n"));
        return OSIP_NOMEM;
    }
    ...
    if (i != 0 && !sipfrag)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"Could not parse start line of message.\n"));
        osip_free(beg);
        beg = NULL;
        return i;
    }
    ...
    if (i != 0)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"error in msg_headers_parse()\n"));
        osip_free(beg);
        beg = NULL;
        return i;
    }
    ...
    if (i != 0)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"error in msg_osip_body_parse()\n"));
        return i;
    }
