...
#ifdef WIN32
    ll_now = (LONGLONG) lSystemTimeAsFileTime.dwLowDateTime + ((LONGLONG)(lSystemTimeAsFileTime.dwHighDateTime) << (LONGLONG)32);
#else
...
osip_gettimeofday(struct timeval* tp, void* tz)
{
    ...
    {
        gettimeofday(tp, (struct timezone*)tz);
        return 0;
    }
...
osip_compensatetime()
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_WARNING, NULL, (char*)"adjusting exosip monotonic time (%i)!\n", diff_real.tv_sec - diff_monotonic.tv_sec));
