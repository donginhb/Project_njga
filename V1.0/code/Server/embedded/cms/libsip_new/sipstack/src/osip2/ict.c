...
__osip_ict_init(osip_ict_t** ict, osip_t* osip, osip_message_t* invite)
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"allocating ICT context\n"));
...
__osip_ict_free(osip_ict_t* ict)
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"free ict ressource\n"));

    if (NULL != ict->destination)
    {
