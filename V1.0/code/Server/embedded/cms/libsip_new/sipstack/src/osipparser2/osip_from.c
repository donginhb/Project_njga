...
osip_from_parse(osip_from_t* from, const char* hvalue)
{
    ...
    if (url_end == NULL)
    {
        /* sip:amoizard@antisip.com;tag=023 *//* We are sure ';' is the delimiter for from-parameters */
        char* host = (char*)strchr(url, '@');

        if (host != NULL)
        {
            gen_params = (char*)strchr(host, ';');
        }
        else
        {
            gen_params = (char*)strchr(url, ';');
        }
    ...
    else                          /* jack <sip:amoizard@antisip.com;user=phone>;tag=azer */
    {
        ...
        gen_params = (char*)strchr(url_end, ';');
