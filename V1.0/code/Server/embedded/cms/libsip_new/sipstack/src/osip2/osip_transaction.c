...
osip_transaction_init(osip_transaction_t** transaction, osip_fsm_type_t ctx_type, osip_t* osip, osip_message_t* request)
{
    ...
    *transaction = (osip_transaction_t*) osip_malloc(sizeof(osip_transaction_t));
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"allocating transaction ressource %i %s\n", (*transaction)->transactionid, request->call_id->number));
    ...
    topvia = (osip_via_t*)osip_list_get(&request->vias, 0);
    ...
    if (ctx_type == ICT)
    {
        ...
        if (i != 0)
        {
            osip_transaction_free(*transaction);
            *transaction = NULL;
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_ict_init error\n"));
            return i;
        }

        __osip_add_ict(osip, *transaction);
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_add_ict ok\n"));
    }
    else if (ctx_type == IST)
    {
        ...
        if (i != 0)
        {
            osip_transaction_free(*transaction);
            *transaction = NULL;
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_ist_init error\n"));
            return i;
        }

        __osip_add_ist(osip, *transaction);
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_add_ist ok\n"));
    }
    else if (ctx_type == NICT)
    {
        ...
        if (i != 0)
        {
            osip_transaction_free(*transaction);
            *transaction = NULL;
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_nict_init error\n"));
            return i;
        }

        __osip_add_nict(osip, *transaction);
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_add_nict ok\n"));
    }
    else
    {
        ...
        if (i != 0)
        {
            osip_transaction_free(*transaction);
            *transaction = NULL;
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_nist_init error\n"));
            return i;
        }

        __osip_add_nist(osip, *transaction);
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"__osip_add_nist ok\n"));
    }
...
osip_transaction_free(osip_transaction_t* transaction)
{
    ...
    i = osip_remove_transaction((osip_t*)transaction->config, transaction);

    if (i != 0)                   /* yet removed ??? */
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"transaction already removed from list %i!\n", transaction->transactionid));
    }
...
osip_transaction_free2(osip_transaction_t* transaction)
{
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"free transaction ressource %i %s\n", transaction->transactionid, transaction->orig_request->call_id->number));
    }
    ...
    if (transaction->transactionff != NULL)
    {
        evt = (osip_event_t*)osip_fifo_tryget(transaction->transactionff);
...
{
    osip_statemachine_t* statemachine = NULL;
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"sipevent tr->transactionid: %i\n", transaction->transactionid));
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"sipevent tr->state: %i\n", transaction->state));
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"sipevent evt->type: %i\n", evt->type));
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"sipevent transaction->ctx_type: %i\n", transaction->ctx_type));
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"sipevent evt->sip: %x\n", evt->sip));
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO3, NULL, (char*)"USELESS event!\n"));
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"sipevent evt: method called!\n"));
    }
...
__osip_transaction_matching_response_osip_to_xict_17_1_3(osip_transaction_t* tr, osip_message_t* response)
{
    ...
    if (b_request == NULL)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_BUG, NULL, (char*)"You created a transaction without any branch! THIS IS NOT ALLOWED\n"));
        return OSIP_SYNTAXERROR;
    }
    ...
    if (b_response == NULL)
    {
        ...
        return OSIP_SUCCESS;
#else
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_BUG, NULL, (char*)"Remote UA is not compliant: missing a branch parameter in  Via header!\n"));
        return OSIP_SYNTAXERROR;
#endif
    }
...
__osip_transaction_need_timer_x_event(void* xixt, struct timeval* timer, int cond_state, int transactionid, int TIMER_VAL)
{
    ...
    if (cond_state)
    {
        ...
        {
            return __osip_event_new((type_t)TIMER_VAL, transactionid);
        }
    }
