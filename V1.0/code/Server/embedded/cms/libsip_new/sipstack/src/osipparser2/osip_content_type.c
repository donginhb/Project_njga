...
osip_content_type_parse(osip_content_type_t* content_type, const char* hvalue)
{
    ...
    subtype = (char*)strchr(hvalue, '/');
    osip_content_type_params = (char*)strchr(hvalue, ';');
    ...
    *dest = NULL;
    ...
    tmp = tmp + strlen(tmp);
    {
        ...
        while (!osip_list_eol(&content_type->gen_params, pos))
        {
            ...
            u_param = (osip_generic_param_t*) osip_list_get(&content_type->gen_params, pos);
            ...
            if (len < tmp_len)
            {
                buf = (char*)osip_realloc(buf, tmp_len);
                len = tmp_len;
                tmp = buf + strlen(buf);
            }
...
osip_content_type_free(osip_content_type_t* content_type)
{
    ...
    if (NULL != content_type->type)
    {
        osip_free(content_type->type);
        content_type->type = NULL;
    }

    if (NULL != content_type->subtype)
    {
