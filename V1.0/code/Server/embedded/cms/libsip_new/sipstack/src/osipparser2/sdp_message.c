...
sdp_key_free(sdp_key_t* key)
{
    ...
    if (NULL != key->k_keytype)
    {
        osip_free(key->k_keytype);
        key->k_keytype = NULL;
    }

    if (NULL != key->k_keydata)
    {
...
sdp_attribute_free(sdp_attribute_t* attribute)
{
    ...
    if (NULL != attribute->a_att_field)
    {
        osip_free(attribute->a_att_field);
        attribute->a_att_field = NULL;
    }

    if (NULL != attribute->a_att_value)
    {
...
sdp_connection_free(sdp_connection_t* connection)
{
    ...
    if (NULL != connection->c_nettype)
    {
        osip_free(connection->c_nettype);
        connection->c_nettype = NULL;
    }

    if (NULL != connection->c_addrtype)
    {
        osip_free(connection->c_addrtype);
        connection->c_addrtype = NULL;
    }

    if (NULL != connection->c_addr)
    {
        osip_free(connection->c_addr);
        connection->c_addr = NULL;
    }

    if (NULL != connection->c_addr_multicast_ttl)
    {
        osip_free(connection->c_addr_multicast_ttl);
        connection->c_addr_multicast_ttl = NULL;
    }

    if (NULL != connection->c_addr_multicast_int)
    {
...
sdp_media_free(sdp_media_t* media)
{
    ...
    if (NULL != media->m_media)
    {
        osip_free(media->m_media);
        media->m_media = NULL;
    }

    if (NULL != media->m_port)
    {
        osip_free(media->m_port);
        media->m_port = NULL;
    }

    if (NULL != media->m_number_of_port)
    {
        osip_free(media->m_number_of_port);
        media->m_number_of_port = NULL;
    }

    if (NULL != media->m_proto)
    {
...
__osip_sdp_append_string(char** string, int* size, char* cur, char* string_osip_to_append)
{
    ...
    if (cur - (*string) + length > *size)
    {
        ...
        (*string) = (char*)osip_realloc((*string), *size + length + 10);
        *size = *size + length + 10;
...
sdp_message_parse_v(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    sdp->v_version = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_s(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    sdp->s_name = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_i(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    /* text is interpreted as ISO-10646 UTF8! */
    /* using ISO 8859-1 requires "a=charset:ISO-8859-1 */
    i_info = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_u(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    sdp->u_uri = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_y(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    /* y=ssrc */
    sdp->y_ssrc = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_f(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    /* f=param */
    sdp->f_param = (char*)osip_malloc(crlf - (equal + 1) + 1);
    ...
    osip_strncpy(sdp->f_param, equal + 1, crlf - (equal + 1));
...
sdp_message_parse_e(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    e_email = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_p(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    /* e=email */
    /* we assume this is an EMAIL-ADDRESS */
    p_phone = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_c(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    char* tmp = NULL;
    char* tmp_next = NULL;
    sdp_connection_t* c_header = NULL;
    int i = 0;
...
sdp_message_parse_t (sdp_message_t* sdp, char* buf, char** next)
{
    ...
    char* tmp_next = NULL;
    int i = 0;
    sdp_time_descr_t* t_header = NULL;
...
sdp_message_parse_r(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    r_header = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_z(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    z_header = (char*)osip_malloc(crlf - (equal + 1) + 1);
...
sdp_message_parse_a(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    char* tmp_next = NULL;
    int i = 0;
    sdp_attribute_t* a_attribute = NULL;
    char* colon = NULL;
...
sdp_message_parse_m(sdp_message_t* sdp, char* buf, char** next)
{
    ...
    *next = buf;
    ...
    slash = (char*)strchr(tmp, '/');
    space = (char*)strchr(tmp, ' ');
...
sdp_message_parse(sdp_message_t* sdp, const char* buf)
{
    ...
    {
        /* return -1; */
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"The \"s\" parameter is mandatory, but this packet does not contain any! - anyway, we don't mind about it.\n"));
    }
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"The rfc2327 says there should be at least an email or a phone header!- anyway, we don't mind about it.\n"));
    }
...
sdp_message_to_str(sdp_message_t* sdp, char** dest)
{
    ...
    *dest = NULL;
    ...
    if (sdp->f_param != NULL)
    {
        ...
        tmp = __osip_sdp_append_string(&string, &size, tmp, sdp->f_param);
        tmp = __osip_sdp_append_string(&string, &size, tmp, CRLF);
    }
...
sdp_message_free(sdp_message_t* sdp)
{
    ...
    if (NULL != sdp->o_sess_id)
    {
        osip_free(sdp->o_sess_id);
        sdp->o_sess_id = NULL;
    }

    if (NULL != sdp->o_sess_version)
    {
        osip_free(sdp->o_sess_version);
        sdp->o_sess_version = NULL;
    }

    if (NULL != sdp->o_nettype)
    {
        osip_free(sdp->o_nettype);
        sdp->o_nettype = NULL;
    }

    if (NULL != sdp->o_addrtype)
    {
