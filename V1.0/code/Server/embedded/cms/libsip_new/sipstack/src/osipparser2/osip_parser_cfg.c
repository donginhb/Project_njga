...
parser_init(void)
{
    ...
    for (i = 0; i < NUMBER_OF_HEADERS; i++)
    {
        ...
        else
        {
            ...
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"conflict with current hashtable size\n"));
...
__osip_message_call_method(int i, osip_message_t* dest, const char* hvalue)
{
    ...
    if (err < 0)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_WARNING, NULL, (char*)"Could not set header: %s: %s\n", pconfig[i].hname, hvalue));
    }
