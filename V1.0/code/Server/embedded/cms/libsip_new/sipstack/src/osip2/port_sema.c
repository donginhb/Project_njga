...
osip_mutex_init()
{
    ...
    /* 当一个unix锁的owner死掉后，其它线程再去加这个锁的时候，不会被阻塞，而是通过返回值EOWNERDEAD来报告错误，那么你可以根据这个错误来进行处理 */
...
int
osip_mutex_trylock(struct osip_mutex* _mut)
{
    osip_mutex_t* mut = (osip_mutex_t*) _mut;
...
int
osip_debug_mutex_lock(struct osip_mutex* _mut, const char* file, int line, const char* func)
{
    int iRet = 0;
    osip_mutex_t* mut = (osip_mutex_t*) _mut;
...
int
osip_debug_mutex_trylock(struct osip_mutex* _mut, const char* file, int line, const char* func)
{
    int iRet = 0;
    osip_mutex_t* mut = (osip_mutex_t*) _mut;
    ...
    iRet = pthread_mutex_trylock(mut);

    if (0 != iRet)
    {
        //printf("\r\n$$$$$$$$$$%s:%d:%s:osip_debug_mutex_trylock:iRet=%d$$$$$$$$$$\r\n", file, line, func, iRet);
        /* fprintf(stdout, "\r\n%s:%d:%s:osip_debug_mutex_trylock:iRet=%d\r\n", file, line, func, iRet); */
        /* fflush(stdout); */
    }
...
int
osip_debug_mutex_unlock(struct osip_mutex* _mut, const char* file, int line, const char* func)
{
    int iRet = 0;
    osip_mutex_t* mut = (osip_mutex_t*) _mut;
    ...
    iRet = pthread_mutex_unlock(mut);

    if (0 != iRet)
    {
        printf("\r\n##########%s:%d:%s:osip_debug_mutex_unlock:iRet=%d##########\r\n", file, line, func, iRet);
        /* fprintf(stdout, "\r\n%s:%d:%s:osip_debug_mutex_unlock:iRet=%d\r\n", file, line, func, iRet); */
        /* fflush(stdout); */
    }
