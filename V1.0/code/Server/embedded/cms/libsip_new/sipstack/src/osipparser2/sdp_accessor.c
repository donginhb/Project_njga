...
int
sdp_message_o_username_set(sdp_message_t* sdp, char* name)
{
    if (sdp == NULL || NULL == name)
    {
        return -1;
    }
...
int
sdp_message_o_sess_id_set(sdp_message_t* sdp, char* name)
{
    if (sdp == NULL || NULL == name)
    {
        return -1;
...
int
sdp_message_o_sess_version_set(sdp_message_t* sdp, char* name)
{
    if (sdp == NULL || NULL == name)
    {
        return -1;
...
int
sdp_message_y_ssrc_set(sdp_message_t* sdp, char* ssrc)
{
    if (sdp == NULL)
    {
        return OSIP_BADPARAMETER;
...
int
sdp_message_f_param_set(sdp_message_t* sdp, char* fparam)
{
    if (sdp == NULL)
    {
        return OSIP_BADPARAMETER;
...
char*
sdp_message_y_ssrc_get(sdp_message_t* sdp)
{
    if (sdp == NULL)
    {
        return NULL;
...
char*
sdp_message_f_param_get(sdp_message_t* sdp)
{
    if (sdp == NULL)
    {
        return NULL;
...
sdp_message_c_nettype_get(sdp_message_t* sdp, int pos_media, int pos)
{
    ...
    conn = sdp_message_connection_get(sdp, pos_media, pos);
...
sdp_message_b_bwtype_get(sdp_message_t* sdp, int pos_media, int pos)
{
    ...
    band = sdp_message_bandwidth_get(sdp, pos_media, pos);
...
sdp_message_a_attribute_del(sdp_message_t* sdp, int pos_media, char* att_field)
{
    ...
    if (pos_media == -1)
    {
        ...
        {
            attr = (sdp_attribute_t*)osip_list_get(&sdp->a_attributes, i);
...
sdp_message_a_attribute_del_at_index(sdp_message_t* sdp, int pos_media, char* att_field, int pos_attr)
{
    ...
    if (pos_media == -1)
    {
        ...
        else if ((attr = (sdp_attribute_t*)osip_list_get(&sdp->a_attributes, pos_attr)) != NULL)
        {
    ...
    for (i = 0; i < osip_list_size(&med->a_attributes);)
    {
        ...
        else if ((attr = (sdp_attribute_t*)osip_list_get(&med->a_attributes, pos_attr)) != NULL)
        {
...
{
    sdp_media_t* med = (sdp_media_t*)osip_list_get(&sdp->m_medias, pos_media);
...
sdp_message_m_payload_del(sdp_message_t* sdp, int pos_media, int pos)
{
    ...
    if ((payload = (char*)osip_list_get(&med->m_payloads, pos)) == NULL)
    {
        return OSIP_UNDEFINED_ERROR;
    }
