...
__osip_nist_init(osip_nist_t** nist, osip_t* osip, osip_message_t* invite)
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"allocating NIST context\n"));

    *nist = (osip_nist_t*) osip_malloc(sizeof(osip_nist_t));

    if (*nist == NULL)
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"malloc NIST error\n"));
        return OSIP_NOMEM;
    }
    ...
    /* for INVITE retransmissions */
    {
        ...
        if (i < 0)
        {
            osip_free(*nist);
            *nist = NULL;
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"message get via NIST error\n"));
            return i;
        }
        ...
        if (proto == NULL)
        {
            osip_free(*nist);
            *nist = NULL;
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"get protocol NIST error\n"));
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"NIST init ok\n"));
    return OSIP_SUCCESS;
}
...
__osip_nist_free(osip_nist_t* nist)
{
    ...
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL, (char*)"free nist resource\n"));
