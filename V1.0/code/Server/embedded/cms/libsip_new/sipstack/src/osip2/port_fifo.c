...
osip_fifo_get(osip_fifo_t* ff)
{
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"no element in fifo.\n"));
#ifndef OSIP_MONOTHREAD
        osip_mutex_unlock(ff->qislocked);
#endif
...
osip_fifo_tryget(osip_fifo_t* ff)
{
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, (char*)"no element in fifo.\n"));
        osip_mutex_unlock(ff->qislocked);
        return OSIP_SUCCESS;
    }
