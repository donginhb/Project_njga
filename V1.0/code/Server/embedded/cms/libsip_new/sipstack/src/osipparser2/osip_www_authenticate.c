...
osip_www_authenticate_free(osip_www_authenticate_t* www_authenticate)
{
    ...
    if (NULL != www_authenticate->realm)
    {
        osip_free(www_authenticate->realm);
        www_authenticate->realm = NULL;
    }

    if (NULL != www_authenticate->domain)
    {
    ...
    if (NULL != www_authenticate->opaque)
    {
        osip_free(www_authenticate->opaque);
        www_authenticate->opaque = NULL;
    }

    if (NULL != www_authenticate->stale)
    {
    ...
    if (NULL != www_authenticate->qop_options)
    {
        osip_free(www_authenticate->qop_options);
        www_authenticate->qop_options = NULL;
    }
