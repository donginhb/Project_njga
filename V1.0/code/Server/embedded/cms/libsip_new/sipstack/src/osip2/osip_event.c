...
osip_parse(const char* buf, size_t length)
{
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, (char*)"could not parse message\n"));
    ...
    else
    {
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO3, NULL, (char*)"MESSAGE REC. CALLID:%s\n", se->sip->call_id->number));
        }
