...
int stolowercase(char* word)
{
    ...
    for (i = 0; i <= len - 1; i++)
    {
        if ('A' <= tmpword[i] && tmpword[i] <= 'Z')
        {
            tmpword[i] = tmpword[i] + 32;
        }
    }
...
int sclrspace(char* word)
{
    ...
    */
    ...
    if (pbeg != tmpword)
    {
        memmove(tmpword, pbeg, pend - pbeg + 2);
    }
...
char* osip_getcopy(const char* ch)
{
    char* copy = NULL;
...
char* osip_getcopy_unquoted_string(const char* ch)
{
    char* copy = NULL;
...
__osip_set_next_token(char** dest, char* buf, int end_separator, char** next)
{
    ...
    *dest = (char*)osip_malloc(sep - (buf) + 1);
...
osip_enquote(const char* s)
{
    ...
    t = rtn = (char*)osip_malloc(strlen(s) * 2 + 3);
