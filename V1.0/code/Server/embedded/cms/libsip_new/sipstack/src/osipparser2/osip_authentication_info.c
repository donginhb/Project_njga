...
osip_authentication_info_free(osip_authentication_info_t* authentication_info)
{
    ...
    if (NULL != authentication_info->nextnonce)
    {
        osip_free(authentication_info->nextnonce);
        authentication_info->nextnonce = NULL;
    }

    if (NULL != authentication_info->rspauth)
    {
        osip_free(authentication_info->rspauth);
        authentication_info->rspauth = NULL;
    }

    if (NULL != authentication_info->cnonce)
    {
        osip_free(authentication_info->cnonce);
        authentication_info->cnonce = NULL;
    }

    if (NULL != authentication_info->nonce_count)
    {
        osip_free(authentication_info->nonce_count);
        authentication_info->nonce_count = NULL;
    }

    if (NULL != authentication_info->qop_options)
    {
        osip_free(authentication_info->qop_options);
        authentication_info->qop_options = NULL;
    }
